...
dir.create("TCGA_LUSC_Data")
setwd("TCGA_LUSC_Data")
...
total_samples <- 450
save_prefix <- "TCGA_LUSC_counts_batch"  # Prefix for saved files
...
batch_files <- list.files(pattern = "TCGA_LUSC_counts_batch_.*\\.rda$")
all_counts <- list()
all_metadata <- list()
...
lapply(all_metadata, colnames)
str(all_metadata)
common_cols <- Reduce(intersect, lapply(all_metadata, colnames))
all_metadata <- lapply(all_metadata, function(df) df[, common_cols, drop = FALSE])
final_metadata <- do.call(rbind, all_metadata)

# 3️⃣ Save final merged dataset
save(final_counts, final_metadata, file = "TCGA_LUSC_counts_FINAL.rda")

# ✅ Done! Use `load("TCGA_LUAD_counts_FINAL.rda")` to reload later.
# 1️⃣ Load the merged dataset
load("TCGA_LUSC_counts_FINAL.rda")  # Loads final_counts & final_metadata
...
gc()  # Free up memory
load("TCGA_LUSC_counts_FINAL.rda")  # Load the dataset
ls()
...
write.csv(upregulated, "Upregulated_genes_LUSC.csv", row.names = FALSE)
...
write.csv(downregulated, "Downregulated_genes_LUSC.csv", row.names = FALSE)

# Save DEGs for future analysis
write.csv(deg_list, "TCGA_LUSC_DEGs.csv", row.names = FALSE)
...
ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Significance)) +
  ...
  theme_minimal() +
  labs(title = "Volcano Plot of LUSC DEGs", x = "log2 Fold Change", y = "-log10 Adjusted p-value")
...
plot(A, M, 
     ...
     main = "MA Plot for LUSC DEGs (Primary Tumor vs Solid Tissue Normal)")
...
# Visualize results
dotplot(gsea_go, showCategory = 20, label_format = 70) + 
...
# Convert lung_cancer_gene_symbols_kegg into a dataframe
...
# Define LUAD biomarkers
luad_biomarker <- c("CDKN1A", "CCNE1", "E2F1", "SKP2", "E2F3", "TRAF4", 
                    "LAMB3", "NFKBIA", "AKT3", "PIK3R1", "CDKN2B", "RXRG", 
                    "MET", "EPAS1", "KIF5A", "EGF", "TERT", "NQO1")
# Define LUSC biomarkers
lusc_biomarker <- c("DVL3", "AXIN2", "CCNE1", "SKP2", "LAMC2", "E2F1", 
                    "CDK4", "LAMB3", "E2F3", "CDK6", "TRAF4", "CYCS", 
                    "BIRC3", "NFKBIA", "RXRG", "TERT", "GSTM1", "NQO1", 
                    "EGFR", "PRKCB", "KIF5A", "EGF", "RASSF1", "EPAS1")

# Find common genes
common_genes <- intersect(luad_biomarker, lusc_biomarker)
...
# Get Entrez IDs from org.Hs.eg.db
entrez_ids <- mapIds(org.Hs.eg.db, keys = common_genes, 
                     column = "ENTREZID", keytype = "SYMBOL", multiVals = "first")

# Create DataFrame
df <- data.frame(Gene_Symbol = common_genes, Entrez_ID = entrez_ids, stringsAsFactors = FALSE)

# Print the dataframe
print(df)

# Save to CSV
write.csv(df, "common_biomarkers.csv", row.names = FALSE)
